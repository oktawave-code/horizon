...
static sgx_status_t SGX_CDECL sgx_ecall_invoke(void* pms)
{
	...
	const char* _tmp_ip = ms->ms_ip;
	size_t _len_ip = ms->ms_ip_len ;
	char* _in_ip = NULL;
	uint8_t* _tmp_output = ms->ms_output;
	uint32_t* _tmp_output_len = ms->ms_output_len;
	...
	if (_tmp_ip != NULL && _len_ip != 0) {
		...
		if (memcpy_s((void*)_in_ip, _len_ip, _tmp_ip, _len_ip)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_ip[_len_ip - 1] = '\0';
		if (_len_ip != strlen(_in_ip) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = ecall_invoke(ms->ms_operation, _tmp_input, ms->ms_input_len, (const char*)_in_ip, _tmp_output, _tmp_output_len);
err:
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_ecall_invoke, 0},
	}
};

SGX_EXTERNC const struct {
	size_t nr_ocall;
	uint8_t entry_table[21][1];
} g_dyn_entry_table = {
	21,
	{
		{0, },
